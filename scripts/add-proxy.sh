#!/usr/bin/env bash
# Optional: if you front the stack with Nginx Proxy Manager, this adds a proxy host (forward -> uchiyomi:3000
...
FORWARD_HOST="${FORWARD_HOST:-uchiyomi}"
...
docker run --rm --network proxy_internal \
  ...
  -v "$DIR/scripts":/s:ro \
  ghcr.io/angelosha/uchiyomi:latest node /s/npm-setup.cjs
