 ...
 * filters nothing itself, and carries the same "not downloaded" size text AND the same page count of 0 as a
 * ghost, so the row says what it is in the list and describes itself the way the other kind of absent chapter
 * does. Off, a tombstone keeps its ERROR status, its stored page count and the route's filter.
 */
export function komgaBook(dto: OwnedBookDto, opts?: { sizeBytes?: number | null; created?: Date | string | number | null; absent?: boolean }) {
  ...
  return {
    ...
    media: {
      ...
      // ⚠️ 0 for a listed tombstone, exactly as a ghost reports 0. The file is gone and `/pages` answers `[]`,
      // so keeping the deleted file's count would advertise twenty pages and open to none -- and the two kinds
      // of absent chapter have to describe themselves the same way or only one of them is honest.
      pagesCount: absent ? 0 : int(dto.media?.pagesCount),
      mediaProfile: 'DIVINA',
 ...
 * whole list it sits in -- komgaGhosts.int.test.ts ('a ghost has every field a real chapter has') compares
 * the two key sets over HTTP for exactly that reason. The differences are the three that make it a ghost:
 *
 ...
 *
 * Everything else matches a real chapter deliberately, `mediaType` included. ⚠️ That value is
 * `application/vnd.comicbook+zip` and NOT komgaBook's `application/zip` fallback, because the fallback is
 * unreachable here: lib/ownedCatalog's bookDto (L151) hands every real chapter that comic-book type, so it
 * is what komgaBook emits for all of them, and a ghost saying `application/zip` would be the odd row out in
 * every list it appears in -- the one thing this DTO must never be. komgaGhosts.int.test.ts compares the
 * ghost's type against a real neighbour's rather than against a literal, so the two cannot drift apart.
 *
...
export function komgaGhostBook(g: {
  ...
  return {
    ...
    media: {
      status: 'READY',
      // The type lib/ownedCatalog (L151) gives every real chapter, so a ghost is not the odd row out.
