...
before(async () => {
  ...
  await migrate();
  await q(`DELETE FROM lib_series WHERE folder LIKE 'Zsr/%'`);
  await q(`CREATE OR REPLACE FUNCTION sr_boom() RETURNS trigger AS $$
             BEGIN IF NEW.folder IN ('${BOOM}', 'Zsr/Stuck') THEN RAISE EXCEPTION 'refused for the test: %', NEW.folder; END IF; RETURN NEW; END
           $$ LANGUAGE plpgsql`);
...
after(async () => {
  ...
  await q(`DELETE FROM lib_series WHERE folder LIKE 'Zsr/%'`).catch(() => {});
...
test('a Fetch whose chapter is on disk but cannot be indexed says so on its card', { skip }, async () => {
  ...
  assert.match(c?.reason ?? '', /Chapter 1 is already on disk, but the library scan could not add it/);
