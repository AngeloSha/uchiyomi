// Dependency-free archive helpers for the engine pack (Phase 2: desktop/scripts + desktop/src/engine.ts).
//
// - readZip / extractZip: central-directory reader, stored + deflate, streams each entry (a 174 MB jar never
//   sits in memory), checks CRC-32, restores unix modes and symlinks written by a unix `zip`.
...
// - tarGzEntries: a streaming ustar/pax/GNU reader over gunzip, for Suwayomi's macOS/Linux tarballs.
 ...
 * Returns { files, bytes }.
 */
export async function extractZip(file, dest, map = (n) => n) {
 ...
 */
export async function writeZip(srcDir, outFile, { mtime = new Date(2026, 0, 1) } = {}) {
  const list = [];
 ...
 */
export async function extractTarGz(file, onEntry) {
