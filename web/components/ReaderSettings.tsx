    ...
    <motion.div className="fixed inset-0 z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      >
        ...
        <div className="mb-1 flex items-center justify-between">
          <h3 className="font-display text-lg font-semibold">Reader</h3>
        ...
        <Row label="Fit">
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => set({ fitWidth: true })}
              className={`rounded-2xl border py-3 text-sm ${prefs.fitWidth ? 'border-accent bg-accent-soft text-accent' : 'border-ink-700 text-fog-300'}`}>Fit width</button>
            <button onClick={() => set({ fitWidth: false })}
              className={`rounded-2xl border py-3 text-sm ${!prefs.fitWidth ? 'border-accent bg-accent-soft text-accent' : 'border-ink-700 text-fog-300'}`}>Original</button>
          </div>
        </Row>
      </motion.div>
    </motion.div>
