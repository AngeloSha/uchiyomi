#!/usr/bin/env node
// Compile engine-shim/*.java with `javac --release 21` into uchiyomi-shim.jar.
// Needs a JDK 21+ on PATH or in JAVA_HOME. Usage: node engine/build-shim.mjs [--out <dir>]
...
const shimSrc = path.join(here, '..', 'engine-shim');
...
export function buildShim(outDir = path.join(here, 'build')) {
  ...
  execFileSync(jdkTool('javac'), ['--release', '21', '-encoding', 'UTF-8', '-Xlint:all', '-Werror',
    '-d', classes, path.join(shimSrc, 'EngineShim.java'), path.join(shimSrc, 'IsolatedPreferences.java')], { stdio: 'inherit' });
